...
import ParamTab, { TabPanel } from '@/components/shared/param-tab';
import Image from '@/components/shared/image';
import FeaturedCard from '@/components/rytu/featured-card';
...
import NftDropDown from '@/components/rytu/rytu-dropdown';
import Avatar from '@/components/shared/avatar';
import NftFooter from '@/components/rytu/rytu-footer';
...
export default function RYTUDetails({
  product,
}: {
  product: RYTUDetailsProps;
}) {
  const {
    isAuction,
    image,
  ...
  const id = params.id
  const [selectImage, setSelectImage] = useState<StaticImageData | null>(null);

  useEffect(()=>{
    let selectedProductImage = NFTList.find(product => {
      return product.id === Number(id);
    })?.image;

    setSelectImage(selectedProductImage || null)
    console.log('product.id', selectedProductImage, id);
  }, [id])
    ...
    <>
      <div className="flex flex-grow">
        <div className="mx-auto flex w-full flex-grow flex-col transition-all xl:max-w-[1360px] 4xl:max-w-[1760px]">
          ...
          <div className="relative flex w-full flex-grow flex-col lg:pt-8 justify-between ltr:lg:mr-[60px] ltr:md:ml-auto ltr:md:pl-0 rtl:md:mr-auto rtl:md:pr-0 lg:min-h-[calc(100vh-96px)] lg:w-[460px] ltr:lg:pl-12 rtl:lg:pr-12 xl:w-[510px] ltr:xl:pl-0 rtl:xl:pr-20">
            <div className="block">
              <div className="block">
