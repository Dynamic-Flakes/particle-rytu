...
type MenuItemProps = {
  ...
  isActive?: boolean;
  updateLayout?: (layoutOption: string) => void;
};
      ...
      {dropdownItems?.length ? (
        <>
          <div
            ...
            onClick={() => {setIsOpen(!isOpen); updateLayout}}
          >
        ...
        <ActiveLink
          href={href}
          className={cn(
            'relative flex h-12 items-center whitespace-nowrap rounded-lg px-[0.9rem] text-sm text-gray-500 transition-all hover:text-brand dark:hover:text-white',
            {
              'bg-brand dark:bg-gray-800 dark:hover:bg-gray-800': isActive,
            },
          )}
        ...
        >
          ...
          {href === pathname && (
            <motion.span
              className={cn(
                'absolute bottom-0 left-0 right-0 h-full w-full rounded-lg bg-brand dark:bg-gray-800 opacity-0 shadow-large transition-opacity',
              )}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.5 }}
