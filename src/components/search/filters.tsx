...
import CollectionSelect from '@/components/shared/collection-select-list';
...
export function LanguageSwitcher({ multiLang } : LanguageSwitcherProps) {
  const [selectedItem, setSelectedItem] = useState(multiLang.choices[0]);
  ...
  return (
    <div className="relative">
      <Listbox value={selectedItem} onChange={setSelectedItem}>
        <ListboxButton className="flex h-10 w-auto items-center min-w-[66px] justify-between rounded-lg bg-gray-100 px-4 text-xs text-gray-900 dark:bg-gray-800 dark:text-white sm:min-w-[76px] sm:text-sm lg:h-11">
          {selectedItem.label}
          <ChevronDown className="ltr:ml-2 rtl:mr-2" />
        </ListboxButton>
        <Transition
          ...
          leaveTo="opacity-0 translate-y-2"
        >
          <ListboxOptions className="absolute right-0 z-10 mt-2 min-w-[66px] origin-top-right rounded-lg bg-white p-3 shadow-large dark:shadow-gray-900 dark:bg-light-dark sm:min-w-[76px]">
            {multiLang.choices.map((item) => (
              <ListboxOption key={item.value} value={item} onClick={() => changeLocale(item.value)}>
                {({ selected }) => (
                  <div
                    className={`block cursor-pointer rounded-lg px-3 py-2 text-xs font-medium text-gray-900 transition dark:text-white sm:text-sm text-nowrap  ${
                      selected
                        ? 'my-1 bg-gray-100 dark:bg-gray-800'
                        : 'hover:bg-gray-50 dark:hover:bg-gray-700'
                    }`}
        ...
        {({ checked }) => (
          <span
            className={`flex h-9 cursor-pointer items-center justify-center rounded-lg border border-solid text-center text-sm font-medium uppercase tracking-wide transition-all ${
              checked
                ? 'border-brand bg-brand text-white shadow-button dark:text-black dark:bg-[var(--color-accent)]'
